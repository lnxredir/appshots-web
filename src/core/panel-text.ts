...
function panelAlignX(panelW: number, alignH: 'left' | 'center' | 'right'): number {
  if (alignH === 'left') return Math.round(panelW * TEXT_ALIGN_MARGIN);
  if (alignH === 'right') return Math.round(panelW * (1 - TEXT_ALIGN_MARGIN));
  return Math.round(panelW / 2);
}
...
export async function buildPanelTextSvg(
  ...
  if (usesFreeTextLayout(opts)) {
    ...
    const titleAttrs = buildTextAttrs('title', opts, titleFontSize, customFonts, titleAlignH);
    const subtitleAttrs = buildTextAttrs('subtitle', opts, subtitleFontSize, customFonts, subtitleAlignH);
    ...
    const alignH = opts.textAlignH ?? 'center';
    const x = panelAlignX(panelW, alignH);
    const titleAttrs = buildTextAttrs('title', opts, titleFontSize, customFonts, alignH);
    const subtitleAttrs = buildTextAttrs('subtitle', opts, subtitleFontSize, customFonts, alignH);
    ...
    if (title) {
      ...
      textElements += renderMultilineText(lines, x, y, titleFontSize, 1.15, titleAttrs);
    }

    if (subtitle) {
      y += Math.round(titleFontSize * 0.4) + subtitleFontSize;
      const text = formatTextContent(subtitle, opts.subtitleUppercase);
      textElements += `<text x="${x}" y="${y}" ${subtitleAttrs}>${escapeXml(text)}</text>`;
    }
  } else {
    const alignH = opts.textAlignH ?? 'center';
    const x = panelAlignX(panelW, alignH);
    ...
    if (title) {
      ...
      textElements += renderMultilineText(lines, x, firstLineY, titleFontSize, 1.15, titleAttrs);
    }
  }
